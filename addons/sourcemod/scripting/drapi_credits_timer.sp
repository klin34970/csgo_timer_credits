/*<DR.API CREDITS TIMER> (c) by <De Battista Clint (https://sourcemod.market)*/
/*                                                                           */
...
public int OnTimerRecord(int client, int track, int style, float time, float lasttime, int currentrank, int newrank)
{
	if(timer_record > 0)
	{
		...
		CPrintToChat(client, "%t", "OnTimerRecord", timer_record);
	}
}
